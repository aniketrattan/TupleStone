...
#ifndef TUPLESTONE_COMMON_ENDIAN_H_
#define TUPLESTONE_COMMON_ENDIAN_H_
...
inline uint8_t LoadU8(const uint8_t* src) {
  return src[0];
}
...
inline uint64_t LoadU64LE(const uint8_t* src) {
  return static_cast<uint64_t>(LoadU32LE(src)) | (static_cast<uint64_t>(LoadU32LE(src + 4)) << 32);
}
...
inline void StoreI64LE(uint8_t* dst, int64_t v) {
  StoreU64LE(dst, static_cast<uint64_t>(v));
}
inline int64_t LoadI64LE(const uint8_t* src) {
  return static_cast<int64_t>(LoadU64LE(src));
}
inline void StoreI32LE(uint8_t* dst, int32_t v) {
  StoreU32LE(dst, static_cast<uint32_t>(v));
}
inline int32_t LoadI32LE(const uint8_t* src) {
  return static_cast<int32_t>(LoadU32LE(src));
}
...
inline void StoreF64LE(uint8_t* dst, double v) {
  StoreU64LE(dst, DoubleToBits(v));
}
inline double LoadF64LE(const uint8_t* src) {
  return BitsToDouble(LoadU64LE(src));
}
...
#endif  // TUPLESTONE_COMMON_ENDIAN_H_
